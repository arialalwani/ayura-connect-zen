...
import { Coffee, Utensils, Moon, Leaf, Clock, Star, Check } from "lucide-react";
interface Meal {
  id: string;
  name: string;
  time: string;
      ...
      <Card className="shadow-sm border-0 bg-card">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {mealPlans.map(plan => <div key={plan.id} className={`p-4 rounded-lg border-2 cursor-pointer wellness-transition ${selectedPlan.id === plan.id ? 'border-primary bg-primary-light/50' : 'border-border bg-card hover:bg-secondary/50'}`} onClick={() => setSelectedPlan(plan)}>
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <h3 className="font-medium text-sm mb-1">{plan.name}</h3>
                    <p className="text-xs text-muted-foreground mb-2">{plan.description}</p>
                  </div>
                  {plan.recommended && <Badge variant="secondary" className="bg-primary text-primary-foreground text-xs">
                      <Star className="h-3 w-3 mr-1" />
                      Recommended
                    </Badge>}
                  {selectedPlan.id === plan.id && <Check className="h-4 w-4 text-primary ml-2" />}
                </div>
                
                <div className="flex flex-wrap gap-1 mb-2">
                  <Badge variant="outline" className={`text-xs ${doshaColors[plan.primaryDosha as keyof typeof doshaColors]}`}>
                    {plan.primaryDosha}
                  </Badge>
                  {plan.suitableFor.slice(1).map(suitable => <Badge key={suitable} variant="outline" className="text-xs">
                      {suitable}
                    </Badge>)}
                </div>
      ...
      <Card className="shadow-sm border-0 bg-card">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <Badge variant="outline" className={`${doshaColors[selectedPlan.primaryDosha as keyof typeof doshaColors]} mr-2`}>
            ...
            <div className="text-sm">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-1">
                  {selectedPlan.suitableFor.map(suitable => <Badge key={suitable} variant="outline" className="text-xs">
                      {suitable}
                    </Badge>)}
                </div>
              </div>
